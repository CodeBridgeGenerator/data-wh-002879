...
import dataWHFakerFactory from "./dataWHFakerFactory";
...
const DataWHFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dataWHFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dataWH").create(elem));
...
export default connect(mapState, mapDispatch)(DataWHFakerDialogComponent);
