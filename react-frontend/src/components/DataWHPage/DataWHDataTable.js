...
const DataWHDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate1 = (rowData, { rowIndex }) => <p >{rowData.count}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.data}</p>
const calendar_24Template3 = (rowData, { rowIndex }) => <p>{new Date(rowData.timestamp).toLocaleDateString()}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "dataWH"});
...
<Column field="count" header="Count" body={p_numberTemplate1} filter={selectedFilterFields.includes("count")} hidden={selectedHideFields?.includes("count")}  sortable style={{ minWidth: "8rem" }} />
<Column field="data" header="Data" body={pTemplate2} filter={selectedFilterFields.includes("data")} hidden={selectedHideFields?.includes("data")}  style={{ minWidth: "8rem" }} />
<Column field="timestamp" header="Timestamp" body={calendar_24Template3} filter={selectedFilterFields.includes("timestamp")} hidden={selectedHideFields?.includes("timestamp")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload DataWH Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="dataWH"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search DataWH" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default DataWHDataTable;